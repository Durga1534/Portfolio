...
const SkillsSection: React.FC = () => {
  ...
  const skills = [
    ...
    { 
      ...
      logo: (
        <svg viewBox="0 0 24 24" className="w-8 h-8" fill="currentColor">
          <path d="M5.016 20.567l.81-12.594 2.25 4.054zm12.484 1.426L8.857 4.37c-.288-.506-.985-.532-1.31-.05L5.015 8.567zm-.688-7.98L14.717 2.79c.258-.452.905-.384 1.084.113l1.995 5.56z"/>
        </svg>
      )
    },
    { 
      ...
      logo: (
        <svg viewBox="0 0 24 24" className="w-8 h-8" fill="currentColor">
          <path d="M24 18.588a1.529 1.529 0 01-1.895-.72l-3.45-4.771-.5-.667-4.003 5.444a1.466 1.466 0 01-1.802.708l5.158-6.92-4.798-6.251a1.595 1.595 0 011.9.666l3.576 4.83 3.596-4.81a1.435 1.435 0 011.788-.668L21.708 7.9l-2.522 3.283a.666.666 0 000 .994l4.804 6.412zM.002 11.576l.42-2.075c1.154-4.103 5.858-5.81 9.094-3.27 1.895 1.489 2.368 3.597 2.275 5.973H1.116C.943 16.447 4.005 19.009 7.92 17.7a4.078 4.078 0 002.582-2.876c.207-.666.548-.78 1.174-.588a5.417 5.417 0 01-2.589 3.957c-2.864 1.857-6.5.7-7.938-2.72C.784 14.696.706 13.259.002 11.576z"/>
        </svg>
      )
    },
    { 
  ...
  useEffect(() => {
    if (isInView && skillsRef.current) {
      const balls = skillsRef.current.querySelectorAll('.skill-ball');
      ...
      <div className="max-w-6xl mx-auto">
        ...
        >
          {skills.map((skill, index) => (
            <motion.div
              key={skill.name}
              className="skill-ball group cursor-pointer"
              whileHover={{ 
                scale: 1.3,
                x: "random(-30, 30)",
                y: "random(-30, 30)",
            ...
            >
              ...
              `}>
                {/* Glowing effect */}
                <div className="absolute inset-0 rounded-full opacity-0 group-hover:opacity-30 bg-white transition-opacity duration-300" />
                
                {/* Technology Logo */}
                <div className="text-white relative z-10 mb-1">
                  {skill.logo}
                </div>

                {/* Skill name */}
                <div className="text-xs font-semibold text-white text-center relative z-10 px-2">
                  {skill.name}
                </div>
